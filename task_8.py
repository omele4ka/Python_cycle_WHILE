...
while True:
    counter += 1
    comp_num = first_elem + (last_elem - first_elem) // 2
    choice = int(input(f'Твое число равно, меньше или больше, чем число {comp_num}? 1 — равно, 2 — больше, 3 — меньше: '))
    if choice == 1:
        print(f'Я угадал! Число {comp_num}')
        print(f'Количество попыток: {counter}')
        break
    elif choice == 2:
        first_elem = comp_num + 1
    else:
        last_elem = comp_num - 1
